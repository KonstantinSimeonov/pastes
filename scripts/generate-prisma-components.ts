...
const graph_to_imports = (graph: ReturnType<typeof build_graph>) =>
  graph.map(
    ([k, deps]) => [k, Array.from(new Set(deps), dep => `import "prismjs/components/prism-${dep}"`).join(`\n`)] as const
  )

const write_modules = (imports: ReturnType<typeof graph_to_imports>) => {
  console.log(imports)
  for (const [filename, content] of imports) {
    const filepath = path.join(process.cwd(), `src`, `prism-components`, `${filename}.ts`)
    console.log(
      `Writing to ${filepath}\n`,
      content,
      `\n`
...
write_modules(
  graph_to_imports(
    build_graph(components.languages as Langs)
  )
)
